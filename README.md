...
Langrepl ships with a prebuilt `claude-style-coder` agent that mimics Claude Code's behavior for software development
tasks and a `code-reviewer` agent for code reviews.

### Interactive Chat Mode (Default)

```bash
# Start interactive session (uses claude-style-coder by default)
uv run langrepl

# Use specific agent
uv run langrepl -a claude-style-coder
...
uv run langrepl -s -a claude-style-coder
...
uv run langrepl -s -a claude-style-coder -am ACTIVE
```
...
- [DEEP_AGENT.md](docs/DEEP_AGENT.md) - Deep agent architecture details
- [HIL.md](docs/HIL.md) - Human-in-the-loop workflows
