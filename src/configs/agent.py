...
class BaseAgentConfig(VersionedConfig):
    ...
    @staticmethod
    def _copy_missing_prompts(
        prompt_paths: list[str], working_dir: Path | None = None
    ) -> None:
        """Copy missing prompt files from defaults (sync, called during migration)."""
        if working_dir is None:
            return  # Skip if no working_dir provided
        try:
            ...
            for prompt_path in prompt_paths:
                ...
                target_file = working_dir / CONFIG_DIR_NAME / prompt_path
    ...
    @classmethod
    def migrate(
        cls, data: dict, from_version: str, working_dir: Path | None = None
    ) -> dict:
        ...
        if from_ver < pkg_version.parse("2.2.0") and (
            ...
            if isinstance(compression, dict):
                ...
                cls._copy_missing_prompts(default_prompts, working_dir)
...
class BatchSubAgentConfig(BaseBatchConfig):
    ...
    async def from_yaml(
        ...
        batch_llm_config = await BatchLLMConfig.from_yaml(working_dir)

        subagents = []
        file_path = working_dir / CONFIG_SUBAGENTS_FILE_NAME
        dir_path = working_dir / CONFIG_SUBAGENTS_DIR
        prompt_base_path = working_dir / CONFIG_DIR_NAME

        if file_path.exists():
            subagents.extend(
                await _load_single_file(
                    file_path, "agents", SubAgentConfig, working_dir
                )
            )
        ...
        for subagent in subagents:
            if prompt_content := subagent.get("prompt", ""):
                subagent["prompt"] = await load_prompt_content(
                    prompt_base_path, prompt_content
                )
    ...
    async def update_subagent_llm(
        ...
        if dir_path.exists():
            subagent_file = dir_path / f"{subagent_name}.yml"
            if subagent_file.exists():
                yaml_content = await asyncio.to_thread(subagent_file.read_text)
                data = yaml.safe_load(yaml_content)
                data["llm"] = new_llm_name
                yaml_str = yaml.dump(data, default_flow_style=False, sort_keys=False)
                await asyncio.to_thread(subagent_file.write_text, yaml_str)
                return
        ...
        if file_path.exists():
            ...
            for agent in agents:
                if agent.get("name") == subagent_name:
...
class BatchAgentConfig(BaseBatchConfig):
    ...
    async def update_agent_llm(
        ...
        dir_path = working_dir / CONFIG_AGENTS_DIR
        file_path = working_dir / CONFIG_AGENTS_FILE_NAME
    ...
    @staticmethod
    async def update_default_agent(working_dir: Path, agent_name: str):
        dir_path = working_dir / CONFIG_AGENTS_DIR
        file_path = working_dir / CONFIG_AGENTS_FILE_NAME
    ...
    async def from_yaml(
        ...
        batch_llm_config = await BatchLLMConfig.from_yaml(working_dir)
        batch_checkpointer_config = await BatchCheckpointerConfig.from_yaml(working_dir)
        batch_sandbox_config = await BatchSandboxConfig.from_yaml(working_dir)
        try:
            batch_subagent_config = await BatchSubAgentConfig.from_yaml(working_dir)
        except ValueError:
            batch_subagent_config = None  # No subagents configured
        ...
        if file_path.exists():
            agents.extend(
                await _load_single_file(file_path, "agents", AgentConfig, working_dir)
            )
