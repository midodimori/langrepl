"""Base utilities for configuration loading and versioned configs."""
...
def _migrate_items(
    items: list[dict],
    config_class: type["VersionedConfig"],
    file_path: Path,
    ...
    for item in items:
        ...
        if pkg_version.parse(current_version) < pkg_version.parse(latest_version):
            migrated_item = config_class.migrate(item, current_version, working_dir)
    ...
    for yml_file in yml_files:
        ...
        if config_class:
            migrated_items, needs_save = _migrate_items(
                file_items, config_class, yml_file, working_dir
            )
...
async def _load_single_file(
    ...
    migrated_items, needs_save = _migrate_items(
        items, config_class, file_path, working_dir
    )
