...
from src.configs.enums import LLMProvider
...
class BatchLLMConfig(BaseModel):
    ...
    async def from_yaml(
        ...
        llms = []
        file_path = working_dir / CONFIG_LLMS_FILE_NAME
        dir_path = working_dir / CONFIG_LLMS_DIR

        if file_path.exists():
            llms.extend(
                await _load_single_file(file_path, "llms", LLMConfig, working_dir)
            )

        if dir_path.exists():
            llms.extend(
                await _load_dir_items(
                    dir_path, config_class=LLMConfig, working_dir=working_dir
                )
            )
