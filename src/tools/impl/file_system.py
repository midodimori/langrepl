...
def _resolve_path(working_dir: str, file_path: str) -> Path:
    """Resolve a file path relative to working directory if not absolute."""
    if Path(file_path).is_absolute():
        return Path(file_path).resolve()
    return (Path(working_dir) / file_path).resolve()
...
def _render_diff_args(args: dict, config: RunnableConfig) -> str:
    ...
    if working_dir and file_path:
        try:
            path = _resolve_path(working_dir, file_path)
            if path.exists():
...
async def create_dir(
    ...
    if not working_dir:
        raise ToolException("Working directory is not configured.")
    path = _resolve_path(working_dir, dir_path)
...
async def move_file(
    ...
    if not working_dir:
        raise ToolException("Working directory is not configured.")
    src = _resolve_path(working_dir, source_path)
    dst = _resolve_path(working_dir, destination_path)
...
async def move_multiple_files(
    ...
    for move in move_paths:
        src = _resolve_path(working_dir, move["source"])
        dst = _resolve_path(working_dir, move["destination"])
        dst.parent.mkdir(parents=True, exist_ok=True)
        shutil.move(str(src), str(dst))
