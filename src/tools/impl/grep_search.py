...
def _resolve_search_path(working_dir: str, search_path: str) -> Path:
    """Resolve a search path relative to working directory if not absolute.

    Special case: '/' is treated as the working directory itself, not system root.
    For actual absolute paths, they are used as-is.
    """
    working_path = Path(working_dir).resolve()

    # Special case: treat '/' as current working directory
    if search_path == "/":
        return working_path
    ...
    return (working_path / search_path).resolve()
...
async def grep_search(
    ...
    resolved_path = _resolve_search_path(working_dir, directory_path)
