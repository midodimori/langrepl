...
class TestReferenceCompleter:
    ...
    def test_resolve_refs_no_refs(self, temp_dir):
        ...
        completer = ReferenceCompleter(temp_dir)
        text = "regular text without references"

        result = completer.resolve_refs(text)

        assert result == text

    def test_resolve_refs_with_file_ref(self, temp_dir):
        """Test resolve_refs with file reference."""
        test_file = temp_dir / "test.txt"
        test_file.write_text("content")
        ...
        assert "@:file:test.txt" not in result
    ...
    def test_resolve_refs_with_invalid_type(self, temp_dir):
        """Test resolve_refs with invalid type keeps original."""
        completer = ReferenceCompleter(temp_dir)
        text = "check @:invalid:something"

        result = completer.resolve_refs(text)

        assert result == text

    def test_resolve_refs_preserves_other_text(self, temp_dir):
        """Test resolve_refs preserves text around references."""
        test_file = temp_dir / "test.txt"
        test_file.write_text("content")

        completer = ReferenceCompleter(temp_dir)
        text = "prefix @:file:test.txt suffix"
